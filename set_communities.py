...
def add_community_alg(comm_assignment_dict, com):
    ...
    for community in range(len(com.communities)):
        ...
        for node in node_list_per_com:
            # no method params
            # comm_assignment_dict[node][com.method_name+'_'+str(com.method_parameters)] = community
            comm_assignment_dict[node][com.method_name] = community
    
    # not necessary to set all community assignments as attributes
    # nx.set_node_attributes(G, comm_assignment_dict)
...
def get_cdms(num_gt_coms):
    ...
    """
        removed the following cdms:
        - AGDL: overlapping communities
        - Belief: takes too long
        - Ricci: takes too long
        - GA-Net: takes too long
        - kcut: puts everything in 1 community
        - cpm: puts every node in own community
        - surprise: puts every node in own community
        - der: can only predict two communities, can be fixed by adjusting cdlib
    ...
    cdm_parameter = [
        ...
        # (alg.surprise_communities, [], 'Surprise'),
        ...
        # (alg.der, [ensemble.Parameter(name='ncomponents', start=num_gt_coms)], 'DER'),
        ...
        # propagation
        # (alg.agdl, [ensemble.Parameter(name='number_communities', start=num_gt_coms), 
            # ensemble.Parameter(name='kc', start=4)], 'AGDL'),
        # (alg.belief, [], 'Belief'),
